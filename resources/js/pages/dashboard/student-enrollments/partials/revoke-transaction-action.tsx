...
export function RevokeTransactionAction({ item }: RevokeTransactionActionProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const options = {
            onSuccess: () => {
                toast.success(isViolation ? "Pelanggaran berhasil dibatalkan" : "Prestasi berhasil dibatalkan");
                reset();
            },
            onError: () => {
        ...
        <AlertDialog onOpenChange={(open) => !open && reset()}>
            ...
            <AlertDialogContent>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <AlertDialogHeader>
                        <AlertDialogTitle>Batalkan {isViolation ? "Pelanggaran" : "Prestasi"}</AlertDialogTitle>
                        <AlertDialogDescription>
                            Apakah Anda yakin ingin membatalkan transaksi ini? Tindakan ini akan mengembalikan poin siswa.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    ...
                    <div className="space-y-2">
                        ...
                        <Textarea
                            ...
                            onChange={(e) => setData("revoke_reason", e.target.value)}
                            className={errors.revoke_reason ? "border-destructive focus-visible:ring-destructive" : ""}
                            required
                        />
                        {errors.revoke_reason && (
                            <p className="text-sm font-medium text-destructive">{errors.revoke_reason}</p>
                        )}
                    </div>
                    ...
                    <AlertDialogFooter>
                        ...
                        >
                            {processing ? "Memproses..." : "Konfirmasi Pembatalan"}
                        </AlertDialogAction>
                    </AlertDialogFooter>
                </form>
            </AlertDialogContent>
