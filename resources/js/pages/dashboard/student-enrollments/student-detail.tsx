    ...
    return (
        <>
            ...
            <div className="space-y-6">
                        ...
                        <CardTableTitle
                            title={
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-xl font-bold">{studentEnrollment.name}</span>
                                </div>
                            }
                            description={
                                <div className="flex flex-wrap gap-4 mt-1 items-center">
                                    <div className="flex items-center gap-1.5 text-xs">
                                        ...
                                        <span>{studentEnrollment.student_number}</span>
                                    </div>
                                    <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                                        <School className="h-3.5 w-3.5" />
                                        <span>{studentEnrollment.student_class}</span>
                                    </div>
                                    <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                                        <Calendar className="h-3.5 w-3.5" />
                                        <span>TA {studentEnrollment.academic_year}</span>
                                    </div>
                                </div>
                            }
                        />
                        <CardTableActions>
                            <div className="flex items-center gap-2">
                                {studentEnrollment.is_active ? (
                                    <Badge variant="success" className="px-3">Aktif</Badge>
                                ) : (
                    ...
                    <CardTable>
                        <CardTableHeader>
                            ...
                            <CardTableActions>
                                <div className={cn(
                                    "p-2 rounded-lg",
                                    studentEnrollment.remaining_points > 50 ? "bg-primary/10 text-primary" : "bg-destructive/10 text-destructive"
                                )}>
                        ...
                        <CardTableContent className="p-6">
                            <p className={cn(
                                ...
                                studentEnrollment.remaining_points > 50 ? "text-primary" : "text-destructive"
                            )}>
                ...
                <div className="space-y-6">
                    <div className="flex items-center gap-3">
                        <h2 className="text-lg font-bold tracking-tight uppercase text-muted-foreground/80">Riwayat Transaksi Poin</h2>
                        <div className="h-px flex-1 bg-muted/60" />
                    </div>

                    {studentEnrollment.point_transaction_groups && studentEnrollment.point_transaction_groups.length > 0 ? (
                        [...studentEnrollment.point_transaction_groups].reverse().map((group) => (
                            <CardTable key={group.id} className="transition-all hover:ring-1 ring-primary/20">
                                <CardTableHeader className="py-3">
                                    <CardTableTitle
                                        title={
                                            <div className="flex items-center gap-2">
                                                <span className="font-bold bg-muted py-0.5 rounded text-muted-foreground">#{group.sequence}</span>
                                                <span className="font-semibold text-base">Periode Penanganan #{group.sequence}</span>
                                            </div>
                                        }
                                        description={
                                            <div className="flex items-center gap-3 mt-1">
                                                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                                    {group.is_closed ? <CheckCircle2 className="h-3.5 w-3.5 text-success" /> : <Clock className="h-3.5 w-3.5 animate-pulse text-amber-500" />}
                                                    <span>{group.is_closed ? "Transaksi Ditutup" : "Sesi Aktif"}</span>
                                                </div>
                                                {group.has_letter && (
                                                    <div className="flex items-center gap-1.5 text-xs text-primary font-medium">
                                                        <FileText className="h-3.5 w-3.5" />
                                                        <span>Surat Peringatan Terbit</span>
                                                    </div>
                                                )}
                                            </div>
                                        }
                                    ...
                                    <CardTableActions>
                                        {!group.is_closed && (
                                            <Badge variant="success" className="animate-in fade-in">Terbaru</Badge>
                                        )}
                                    </CardTableActions>
                                </CardTableHeader>
                                <CardTableContent>
                        ...
                        <CardTable className="border-dashed border-2 py-10">
                            <div className="flex flex-col items-center justify-center p-6 text-center">
                                ...
                                <h3 className="text-sm font-semibold text-muted-foreground/80">Belum Ada Transaksi Poin</h3>
                                <p className="text-xs text-muted-foreground max-w-[240px] mt-1">Riwayat pelanggaran dan prestasi siswa akan tercantum di sini.</p>
                            </div>
                        </CardTable>
                    )}
                </div>
