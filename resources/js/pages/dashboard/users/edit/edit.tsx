                        ...
                        <CardTableContent className="p-8">
                            <div className="max-w-xs space-y-2">
                                ...
                                <p className="text-xs text-muted-foreground italic">
                                    * Perubahan role akan segera disimpan dan
                                    mungkin mempengaruhi hak akses pengguna.
                                </p>
                            </div>
                        </CardTableContent>
                    </CardTable>
                                ...
                                ([group, permissions]) => (
                                    <div key={group} className="space-y-6">
                                        <h3 className="text-base font-semibold uppercase tracking-wider text-foreground border-b pb-2 flex items-center justify-between">
                                            {group.replace(/-/g, " ")}
                                            <span className="text-sm font-normal text-muted-foreground tracking-normal lowercase italic">
                                                /{group}
                                            </span>
                                        </h3>
                                        <div className="grid gap-3.5">
                                            {permissions.map((permission) => {
                                                ...
                                                return (
                                                    <div
                                                        key={permission}
                                                        className={`flex items-center space-x-4 group ${isRolePermission ? "cursor-not-allowed opacity-70" : "cursor-pointer"}`}
                                                        onClick={handleClick}
                                                    >
                                                        <Checkbox
                                                            id={permission}
...
UserEdit.layout = ({ user }: Props) => [
    AppLayout,
    {
        breadcrumbs: [
            { title: "Dashboard", href: dashboardIndex() },
            { title: "Pengguna", href: usersIndex() },
            { title: user.name, href: usersEdit({ id: user.id }) },
        ],
    },
];
